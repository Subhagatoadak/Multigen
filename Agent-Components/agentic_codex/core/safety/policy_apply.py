...
def apply_rate_limits(manifest: WorkflowManifest, *, limiter: MultiLimiter, hub: CommunicationHub | None = None) -> None:
    ...
    for policy in rate_policies:
        ...
        if "namespace" in targets:
            for ns, cfg in targets["namespace"].items():
                limiter.set_limit("namespace", ns, RateLimiter(rate_per_sec=cfg.get("rate", 1), capacity=cfg.get("capacity", 1)))
                if hub:
                    hub.set_namespace_rate_limit(ns, cfg.get("rate", 1), cfg.get("capacity", 1))
        if "agent" in targets:
            for ag, cfg in targets["agent"].items():
                limiter.set_limit("agent", ag, RateLimiter(rate_per_sec=cfg.get("rate", 1), capacity=cfg.get("capacity", 1)))
        if "tool" in targets:
